...
======================================================
Sections:
  1. Experiment selector + auto-fill from Improve
  2. Visual topic editor (checkbox + difficulty + num_questions per topic)
  3. Generate MCQ (retrieval + pipeline SLURM)
  4. Results viewer + download JSON
  5. Quiz mode → finish → answers + explanations
...
# ── Full topic database (from topic_list_based.json) ─────────────────────────
# Used to build the visual editor — all chapters and topics available
...
def merge_mcqs_with_explanations(mcqs, explanations):
    ...
    for mcq in mcqs:
        ...
        if rec:
            ...
            su = ed.get("sources_used",[]) if isinstance(ed,dict) else []
            sf = rec.get("sources",[])
            mcq["sources"] = su + sf
        enriched.append(mcq)
    return enriched
...
def list_experiments():
    ...
    for d in sorted(OUTPUT_ROOT.iterdir(), key=lambda p: p.stat().st_mtime,
                    reverse=True):
        ...
        exps.append({"name":d.name,"path":d,"done_steps":sorted(ds),
                      "step_counts":sc,"mcq_count":len(mcqs),
                      "mtime":datetime.fromtimestamp(d.stat().st_mtime)})
    return exps

def poll_slurm_until_done(job_id, log_path, pb, stxt, lc):
    """Poll SLURM job. Returns final state. Does NOT call st.rerun()."""
    lpos = 0; ll = []; it = 0; uc = 0
    while True:
        it += 1; state = "UNKNOWN"
        try:
                ...
                if uc >= 2:
                    try:
                        ...
                        if sa.returncode == 0 and sa.stdout.strip():
                            ls = [l.strip() for l in
                                  sa.stdout.strip().split("\n") if l.strip()]
                            if ls:
                                state = ls[0].split("+")[0].upper()
                    except: pass
...
def build_topic_list_from_ui(base_topics, prefix="ed"):
    """Read Streamlit widget values and build topic_list JSON."""
    result = []
    for ch in base_topics:
        ch_id = ch["chapter_id"]
        selected_topics = []
        for t in ch.get("topics", []):
            tid = t["topic_id"]
            if st.session_state.get(f"{prefix}_sel_{tid}", False):
                diff = st.session_state.get(f"{prefix}_diff_{tid}", "G2")
                nq = st.session_state.get(f"{prefix}_nq_{tid}", 2)
...
with c3:
    ...
    if st.button("✨ Áp dụng", use_container_width=True):
        ...
        st.session_state._improve_mode = False
        st.rerun()
...
if st.session_state._improve_topics is not None:
    ...
    # Build filtered topic_list with 5 questions per difficulty
    ...
    for ch in full_topics:
        ch_topics = []
        for t in ch.get("topics", []):
            if t["topic_id"] in weak_ids:
                # 3 entries per weak topic: G1×5, G2×5, G3×5
                for diff, nq in [("G1", 5), ("G2", 5), ("G3", 5)]:
                    ch_topics.append({
                        "topic_id": t["topic_id"],
                        "topic_name": t["topic_name"],
                        "difficulty": diff,
    ...
    st.session_state._improve_mode = True
    st.session_state.generation_phase = None
    ...
    st.success(f"🎯 **Improve mode** — Focus: {', '.join(topic_names)} "
               f"(5×G1 + 5×G2 + 5×G3 mỗi topic)")
    st.info(f"📂 Experiment: `{new_exp}` — Nhấn **Generate** để bắt đầu.")
        ...
        for i, sn in enumerate(row):
            ...
            with cols[i]:
                ...
                st.markdown(
                    ...
                    f'<div style="font-weight:700;font-size:12px">{ic} {m["label"]}'
                    f'</div><div style="font-size:10px;color:#6b7280">{sta}'
                    f'</div></div>', unsafe_allow_html=True)

st.markdown("---")

# ═══════════════════════════════════════════════════════════════════════════════
# 2️⃣ Visual Topic Editor
# ═══════════════════════════════════════════════════════════════════════════════

if st.session_state.topic_data is None:
    st.info("Chọn experiment bên trên.")
else:
    # Weak topics warning from quiz history
    if exp_name:
        ...
        if past:
            ...
            if wl_warn:
                wn = [f"**{v['topic_name']}** ({v['accuracy']*100:.0f}%)"
                      for v in sorted(wl_warn.values(),
                                      key=lambda x: x["accuracy"])]
                st.warning(f"🎯 Topics yếu: {', '.join(wn)}. "
                           f"Dùng nút Improve ở Section 6.")

    st.markdown("#### 2️⃣ Chọn chủ đề & số câu hỏi")

    # Load base topics for the visual editor
    base_topics = load_topic_list_source()

    # Determine which topics are currently selected (from topic_data)
    current_data = st.session_state.topic_edited or st.session_state.topic_data
    selected_map = {}  # topic_id → {difficulty, num_questions}
    if current_data:
        for ch in current_data:
            for t in ch.get("topics", []):
                tid = t["topic_id"]
                selected_map[tid] = {
                    "difficulty": t.get("difficulty", "G2"),
                    "num_questions": t.get("num_questions", 2),
                }

    # ── Render visual editor ─────────────────────────────────────────────────
    diff_options = ["G1", "G2", "G3"]
    diff_labels = {"G1": "🟢 Dễ (G1)", "G2": "🟡 Trung bình (G2)",
                   "G3": "🔴 Khó (G3)"}

    with st.expander("📋 Chọn chương & chủ đề", expanded=True):
        # Header
        hc = st.columns([0.3, 2.5, 1.2, 1, 1])
        with hc[0]: st.markdown("**✓**")
        with hc[1]: st.markdown("**Chủ đề**")
        with hc[2]: st.markdown("**Độ khó**")
        with hc[3]: st.markdown("**Số câu**")
        with hc[4]: st.markdown("")
        ...
        for ch in base_topics:
            ...
            # Chapter header
            st.markdown(f'<div style="background:#f3f4f6;padding:6px 12px;'
                        f'border-radius:6px;margin:8px 0 4px 0;'
                        f'font-weight:700;font-size:14px;color:#374151">'
                        f'📖 {ch_id}: {ch_name}</div>',
            ...
            for t in ch.get("topics", []):
                tid = t["topic_id"]
                tname = t["topic_name"]
                is_selected = tid in selected_map
                defaults = selected_map.get(tid, {
                    "difficulty": t.get("difficulty", "G2"),
                    "num_questions": t.get("num_questions", 2),
                })

                tc = st.columns([0.3, 2.5, 1.2, 1, 1])
                with tc[0]:
                    st.checkbox("", value=is_selected,
                                key=f"ed_sel_{tid}",
                                label_visibility="collapsed")
                with tc[1]:
                    st.markdown(f"<span style='font-size:14px'>{tname}</span>",
                                unsafe_allow_html=True)
                with tc[2]:
                    cur_diff = defaults["difficulty"]
                    di = diff_options.index(cur_diff) if cur_diff in diff_options else 1
                    st.selectbox("Độ khó", diff_options, index=di,
                                 key=f"ed_diff_{tid}",
                                 label_visibility="collapsed")
                with tc[3]:
                    st.number_input("Số câu", min_value=1, max_value=30,
                                    value=defaults["num_questions"],
                                    key=f"ed_nq_{tid}",
                                    label_visibility="collapsed")
                with tc[4]:
                    st.markdown("")
        ...
        with ac1:
            if st.button("✅ Áp dụng chọn lựa", key="apply_visual",
                         use_container_width=True):
                built = build_topic_list_from_ui(base_topics, "ed")
                total_t = sum(len(ch.get("topics",[])) for ch in built)
                total_q = sum(t["num_questions"]
                              for ch in built for t in ch.get("topics",[]))
                if not built:
                    st.error("⚠️ Chưa chọn chủ đề nào!")
                else:
                    st.session_state.topic_edited = built
                    st.session_state.topic_data = built
                    st.success(f"✅ Đã chọn {total_t} chủ đề — "
                               f"tổng {total_q} câu hỏi")
        with ac2:
            if st.button("🔄 Chọn tất cả", key="select_all",
                         use_container_width=True):
                for ch in base_topics:
                    for t in ch.get("topics",[]):
                        st.session_state[f"ed_sel_{t['topic_id']}"] = True
                st.rerun()
        with ac3:
            if st.button("❌ Bỏ chọn tất cả", key="deselect_all",
                         use_container_width=True):
                for ch in base_topics:
                    for t in ch.get("topics",[]):
                        st.session_state[f"ed_sel_{t['topic_id']}"] = False
                st.rerun()

        # Summary
        built_preview = build_topic_list_from_ui(base_topics, "ed")
        total_sel = sum(len(ch.get("topics",[])) for ch in built_preview)
        total_q = sum(t["num_questions"]
                      for ch in built_preview for t in ch.get("topics",[]))
        st.caption(f"📋 Đang chọn: {total_sel} chủ đề — {total_q} câu hỏi")

    # Advanced: raw JSON editor (collapsed)
    with st.expander("🔧 Xem/sửa JSON (nâng cao)", expanded=False):
        cur = st.session_state.topic_edited or st.session_state.topic_data
        ed = st.text_area("JSON", value=json.dumps(cur, ensure_ascii=False,
                          indent=2), height=250, label_visibility="collapsed")
        jc1, jc2 = st.columns(2)
        with jc1:
            if st.button("💾 Áp dụng JSON", key="json_apply"):
                try:
                    p = json.loads(ed)
                    ...
                    st.error(f"❌ {e}")
        with jc2:
            if st.button("↩️ Reset về gốc", key="json_reset"):
                st.session_state.topic_edited = None
                st.session_state.topic_data = load_topic_list_source()
                st.session_state._improve_mode = False
                st.rerun()
    ...
    if gp and st.session_state.generation_phase is None:
        ...
        if not en or not td:
            ...
            with open(PROJECT_ROOT / "input" / "topic_list.json", "w",
                      encoding="utf-8") as f:
                json.dump(td, f, ensure_ascii=False, indent=2)
            ...
            # Phase 1: Retrieval
            st.info("🔄 Retrieval...")
            rv = {**subprocess.os.environ, "EXP_NAME": en,
                  "HF_HUB_OFFLINE":"1", "TRANSFORMERS_OFFLINE":"1",
                  "TOKENIZERS_PARALLELISM":"false", "OMP_NUM_THREADS":"1",
                  "CUDA_VISIBLE_DEVICES":"", "PYTHONPATH": str(PROJECT_ROOT)}
            retrieval_ok = False
            try:
                rr = subprocess.run(
                    ["python","-u",
                     str(PROJECT_ROOT/"src"/"gen"/"retrieval.py")],
    ...
    if mcqs:
        ...
        if not st.session_state.quiz_active and not st.session_state.quiz_finished:
            ...
            if st.button("🏁 Kết thúc làm bài", type="primary",
                         use_container_width=True):
                results = []
                for q in qs:
            ...
            if sd is None:
                ...
                sd = ss[0] if ss else {}
            if sd:
                total=sd.get("total",0); correct=sd.get("correct",0)
                ...
                qmap = {q.get("question_id",""):q
                       ...
                       if st.session_state.quiz_questions else {}
                ...
                for r in sd.get("results",[]):
                    ...
                    st.markdown(f"**{ic} {qid}** — {r.get('topic_name','')} "
                                f"({r.get('difficulty','')})")
                    if q: st.markdown(q.get("question_text",""))

                    opts = q.get("options",{}) if q else {}
                    cs = set(r.get("correct_answers",[])); us = set(r.get("user_answer",[]))
                    if opts:
                        for lb in sorted(opts.keys()):
                            tx = opts[lb]
                            if lb in cs and lb in us:
                                st.markdown(f"&nbsp;&nbsp;✅ **{lb}. {tx}** ← đúng")
                            elif lb in cs:
                                st.markdown(f"&nbsp;&nbsp;🟢 **{lb}. {tx}** ← đáp án đúng")
                            elif lb in us:
                                st.markdown(f"&nbsp;&nbsp;❌ ~~{lb}. {tx}~~ ← sai")
                            else:
                                st.markdown(f"&nbsp;&nbsp;⬜ {lb}. {tx}")
                                    ...
                                    for src in sources:
                                        if isinstance(src,dict):
                                            ...
                                            if stype=="slide" and page:
                                                st.markdown(f"- 📄 {desc} (trang {page})")
                                            elif url:
                                                st.markdown(f"- 🔗 [{desc or url}]({url})")
                                            elif desc:
                                                st.markdown(f"- {desc}")
                    st.markdown("---")
    ...
    if exp_name:
        ...
        if sessions:
            ...
            if wl:
                ...
                st.markdown("**🎯 Topics yếu (< 50% accuracy, ≥ 2 lần):**")
                for tid, info in wl:
                ...
                if len(wl) > 1:
                    if st.button(
                            f"🔧 Improve tất cả {len(wl)} topics yếu "
                            f"(mỗi topic: 5×G1 + 5×G2 + 5×G3)",
            ...
            for sess in sessions[:10]:
                ...
                t = sess.get("total",0); c = sess.get("correct",0)
                a = sess.get("accuracy",0)
                ic = "✅" if a>=0.7 else "⚠️" if a>=0.5 else "❌"
                with st.expander(
                        f"{ic} `{sid}` — {ts} — {c}/{t} ({a*100:.0f}%)"):
                    for r in sess.get("results",[]):
        ...
        if experiments:
            for exp in experiments[:15]:
                ...
                with ec2:
                    st.caption(
                        f"Steps: "
                        f"{', '.join(f'S{s}' for s in exp['done_steps']) or '—'}")
                with ec3:
